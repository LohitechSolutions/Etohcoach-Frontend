...
> {
  ...
  forgotPasswordApiCall() {
    ...
    this.forgotPassApiCallId = requestMessage.messageId;
    requestMessage.addData(
      getName(MessageEnum.RestAPIResponceEndPointMessage),
      configJSON.forgotPasswardApiEndPoint
    );
  ...
  async componentDidMount() {
    ...
    this.setState({language : await AsyncStorage.getItem("appLanguage")}); 
  }
  ...
  async receive(from: string, message: Message) {
    ...
    if (getName(MessageEnum.NavigationPayLoadMessage) === message.id) {
      ...
      if (apiRequestCallId && responseJson) {
        if (apiRequestCallId === this.forgotPassApiCallId) {
          this.setState({ showLoader: false })
          if (responseJson.error) {
              this.setState({error: responseJson.error && responseJson.error?.length > 0 ? responseJson.error[0] : STRINGS.MESSAGE.EMAIL_UNSENT })
              this.clearValidation()
            }
            else {
              // this.props.navigation.navigate('ForgotPasswordOTP')
              this.setState({email: '', successMsg: true})
              setTimeout(() => {
